...
/*   Updated: 2025/07/09 10:49:14 by akurmyza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Channel::handleJoin(Client *client, Server &server)
{
	...
	sendToChannelExcept(client->getNickname() + " has joined the channel.", *client); // Notify other members
...
void Channel::handlePart(Client *client, Server &server)
{
	...
	sendToChannelExcept(client->getNickname() + " has left the channel.",  *client); // Notify other members
...
void Channel::handleTopic(Client *client, const std::string &newTopic)
{
	...
	sendToChannelExcept("Topic changed to: " + _topic,  *client); // Notify other members
...
void Channel::handleMode(Client *client, char mode, bool enable)
{
	...
	sendToChannelExcept("Mode '" + std::string(1, mode) + "' has been " + modeStatus + ".",  *client); // Notify other members
...
void Channel::handleInvite(Client *client, int targetFd)
{
	...
	sendToChannelExcept(client->getNickname() + " has invited a user to the channel.",  *client); // Notify other members
...
void Channel::handleKick(Client *client, int targetFd, const std::string &reason)
{
	...
	sendToChannelExcept(client->getNickname() + " has kicked a user from the channel. Reason: " + reason,  *client); // Notify other members
...
void Channel::handleKey(Client *client, const std::string &key)
{
	...
	sendToChannelExcept("Channel key has been changed",  *client); // Notify other members
